...
**For v2 (LANDED, expanded):** `engine.ValidatePreLaunch` is the new front door. The full check list it now runs at every launch — and that `kilroy workflows validate <name>` exposes as a standalone command:

1. **Package integrity** (when launched from a `--package` / `kilroy run <name>`): `workflow.toml` parses with required fields (`name`/`description`/`version`); every `tool_command bash <path>` references an existing regular non-empty file in the package; every agent `class=` attribute references a real policy class.
2. **Class resolution per agentic node** (Step 4b/5/6 wiring): `class=` resolved through the policy chain against the live machine-state snapshot; resolved tuple (model/driver/transport/auth/turn-codec) recorded.
3. **Auth presence**: the resolved candidate's auth requirement (env var present + healthy, or CLI session OK) confirmed against `auth.ListAll`.
4. **CLI binary capability probe**: for CLI drivers, the resolved binary (`claude`, `codex`, `gemini`) is on PATH AND responds 0 to `--help` within 5s — catches broken downloads / wrong arch / missing dyn deps.
5. **`[secrets].needs` validation**: each manifest-declared secret name (provider name, e.g. `github`, `anthropic`) must have at least one `state=ok` entry in the auth snapshot, otherwise typed failure with a `kilroy auth list` remediation hint.
...
- [x] **`kilroy workflows validate <name>`** runs DOT validation + the full prelaunch suite (package integrity + class resolution + auth + CLI capability + secrets) and exits 1 on any failure. JSON-by-default; `--pretty` for human view.
- [x] **`[secrets].needs` semantics wired** — manifest `[secrets].needs = [...]` is now threaded through `RunOptions.RequiredSecrets` and validated by prelaunch against the `auth.ListAll` snapshot. Failure is typed `PreLaunchError` with a `kilroy auth list` remediation hint. Authored by parallel kilroy worker `01KQMEJRZ5GDHK2R80GF6ETRZH` (7m13s, success); cherry-picked with merge-conflict resolution against the package-integrity work.
...
- [x] **Step 2 (claude-CLI JSONL codec).** `ParseClaudeCLIJSONL(io.Reader) ([]TurnEvent, error)` lands at `internal/attractor/agentbackend/claude_cli_jsonl.go`. Maps every wire shape that appears in production: `system` init (skipped), `assistant.{text,thinking,tool_use}` (one event per content block), `user.tool_result` (driver-relayed; string OR list-of-blocks content), `result` (TurnEnd with stop_reason / usage / cost). Tolerant of malformed lines (interleaved stderr happens) and large lines (16MB ceiling — claude routinely emits multi-100KB lines for tool inputs/outputs). 7 unit tests + a real-capture stress test that walked **203 captured files / 17,724 events / 0 parser errors** on the dev machine. **No agentbackend type changes were required** — every real shape mapped cleanly to existing variants. That's the forcing-function signal: the type design held under real pressure.
- [x] **Step 2 expanded — three more codecs landed via parallel campaign (2026-05-02).** Per §15.4 Campaign C, three codecs authored in parallel and merged:
  - **Anthropic SSE** (`anthropic_sse.go`, commit `d8e2d9d`): SSE state machine with per-block accumulators keyed on `index`; text/thinking/tool_use blocks emit at `content_block_stop`; `message_delta` / `message_stop` emit `TurnEventTurnEnd` with stop_reason + usage. 10 unit tests including malformed-line tolerance + 120KB tool-arg blob.
  - **OpenAI Responses SSE** (`openai_sse.go`, commit `a2534c9`): same SSE pattern; `response.output_text.delta` accumulators keyed on `itemID:contentIndex`; `response.output_item.added` triggers item-type dispatch (message/reasoning/function_call); `response.completed` emits TurnEnd with status + usage. 9 unit tests.
  - **Codex CLI JSONL** (`codex_cli_jsonl.go`, commit `020dee6`): two-phase pairing (`item.started` → `TurnEventToolUse`, `item.completed` → `TurnEventToolResult`) with same item.id linking the pair; `agent_message`/`command_execution`/`file_change` mapped to text/tool_use/tool_result; `turn.completed` emits TurnEnd. 12 unit tests; flagged a real-corpus discrepancy in agentlog/codex.go's `file_change` field assumption (handled gracefully in the new codec).
  - **No agentbackend type changes were required by any of the three.** All four codecs (claude-CLI + Anthropic SSE + OpenAI SSE + codex-CLI) map cleanly to existing TurnEvent variants — the type design has held under four implementations. Gemini stream codec deferred (gemini_cli not yet exercised in shipped workflows).
...
### Block 9: Shipped workflows — **trio LANDED (`fix`, `investigate`, `review`); end-user packaging deferred**

> **Status discipline (revised post-reframe):** Block 9's exit bar shifted with the §10 reframe — there is no longer a "blessed bare-form" milestone; the bar is "the trio is authored, on the v2 manifest schema, and reachable via `kilroy run <name>`." All three meet that bar today. `implement` exists at the same shape but isn't part of the v2 trio. End-user discoverability without `KILROY_WORKFLOW_PATHS` (i.e., the symlink/installer-copy story) is a packaging concern for release time, not a Block 9 deliverable.
