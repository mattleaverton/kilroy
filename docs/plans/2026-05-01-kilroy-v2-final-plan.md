...
- [ ] **Step 4b (CLI/tmux path)** — surfaced by dogfooding the implement workflow: when `--tmux` is set the engine dispatches via `TmuxAgentHandler`, which has its own provider/model attribute reads. Step 4a's wiring doesn't fire there. **Fix options:** (i) duplicate the class-resolution logic in `TmuxAgentHandler`, (ii) extract a shared helper called from both handlers, or (iii) move class resolution up a layer (engine pre-dispatch, mutating `node.Attributes` once before any handler reads them — both handlers benefit, plus future handlers do automatically). Option (iii) is the cleanest but biggest surgery; (ii) is the pragmatic next step.
...
**Implement workflow scaffold landed alongside Step 4a:** `workflows/implement/` exists with `graph.dot`, scripts, prompt template, and a validator-passing topology. The agent node declares `class="hard_coding"` (forward-compatible — Step 4b will make it load-bearing in `--tmux` runs). Live dogfood: ran a tiny task end-to-end in 14.5s; all 5 stages executed correctly; the only gap was that no `policy_class_resolved` event fired (Step 4b above). The workflow itself is production-ready for everyday dogfooding starting now — the policy benefit kicks in once 4b lands.
