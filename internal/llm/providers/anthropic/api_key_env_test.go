// Verifies the anthropic adapter honors the _KILROY-first env precedence
// (ANTHROPIC_API_KEY_KILROY before ANTHROPIC_API_KEY).
...
func TestAPIKeyEnvPrecedence_KILROY(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			set:     map[string]string{"ANTHROPIC_API_KEY_KILROY": "kilroy-secret", "ANTHROPIC_API_KEY": "canonical-secret"},
			wantKey: "kilroy-secret",
		},
		{
			name:    "falls back to canonical",
			set:     map[string]string{"ANTHROPIC_API_KEY": "canonical-secret"},
			wantKey: "canonical-secret",
		},
		{
			name:    "kilroy alone is sufficient",
			set:     map[string]string{"ANTHROPIC_API_KEY_KILROY": "only-kilroy"},
			wantKey: "only-kilroy",
		},
		{
